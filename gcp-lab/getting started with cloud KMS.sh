...
#Task 4. Create a KMS KeyRing and CryptoKey
...
#3. Copy to the bucket
gsutil cp 1.encrypted gs://${BUCKET_NAME}

#Task 6. Configure IAM permissions
#1.  get the usermail of the lab
USER_EMAIL=$(gcloud auth list --limit=1 2>/dev/null | grep '@' | awk '{print $2}')
# 2. Assign permission to manage KMS resources
...
#2. encrypt each file from the google storage bucket
MYDIR=finance-dept
...
#3. Copy the encrypted data to the bucket
